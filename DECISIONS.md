...
| 6 | Edge route | **Rides the existing shared `AI-API-RR` route** (`/ai/*`) per the AMN EdgeTopology standard. Client endpoint is the **AFD apex** `https://api.{env}.amnhealthcare.io/ai/new-relic-mcp/{env}` — APIM is internal-mode, so `*.azure-api.net` is never client-facing. APIM API `path = ai/new-relic-mcp/{env}` so the `<x>=ai` prefix lines up across AFD → AGW → APIM. Resources named `new-relic-mcp` (`api-new-relic-mcp-{env}`, `backend-new-relic-mcp-{env}`) to match the URL segment. No new AFD/AGW config expected (reuse the AI route) — **confirm with the edge team** that the AI-API-RR `patternsToMatch` is a `/ai/*` wildcard and that AGW→APIM liveness is shared (a `type=mcp` API has no per-op `/liveness`). NR's hosted MCP is external SaaS, so APIM egresses out to it rather than via a private endpoint (the one PE-backend deviation). |
